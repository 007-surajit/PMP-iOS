...
function createJobSubmitTableComplete() {
    ...
    localDatabaseInstance.transaction(function insertUserPrefDB(tx) {
                                      ...
                                      localDatabaseInstance = openDatabase("PMP Database", "1.0", "PMP Database", 200000);
                                      //localDatabaseInstance.transaction(function(tx){tx.executeSql("DELETE FROM OUTSTANDINGJOBS WHERE cont_nr = ?",[jobSubmitData.cont_nr])}, function(err){console.log("Error processing SQL: "+err.code+ '+err.message)}, function(){getOutstandingJobs();});
...
function prepareSavedJobsSubmit()
{
	if(navigator.connection) {
		...
		if(networkState != Connection.NONE) {
			localDatabaseInstance = openDatabase("PMP Database", "1.0", "PMP Database", 200000);
			localDatabaseInstance.transaction(querySavedJobs, function(err){console.log("No jobs found in database")});
		}
	}
}

function querySavedJobs(tx) {
    tx.executeSql('SELECT * FROM JOBSUBMIT', [], submitSavedJobs, function(err){console.log("No jobs found in database")});
}
...
function onDeviceReady() {
    ...
    if(checkDataCron == false) {
		...
		submitDeliveryChecksCron = setInterval(prepareSavedDeliveryChecksSubmit,10000);
		submitQueriesCron = setInterval(prepareSavedQueriesSubmit,10000);
		submitQueriesCron = setInterval(prepareSavedJobsSubmit,10000);
    }
